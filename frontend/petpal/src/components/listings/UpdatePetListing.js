...
import { updatePetListing, getPetListingDetails, currentUser } from '../../api'; // Adjust the path accordingly
...
const UpdatePetListing = () => {
  ...
  useEffect(() => {
    const fetchPetListingData = async () => {
      try {
        // Call the getPetListingDetails function from api.js to fetch existing pet listing data
        const petListingData = await getPetListingDetails(listingId);
        const { photos, ...formDataWithoutPhotos } = petListingData;

        setFormData(formDataWithoutPhotos);
      } catch (error) {
        console.error('Error fetching pet listing data:', error);
        // Handle the error, e.g., show an error message to the user
      }
    };

    // Only fetch data if the user is logged in and is a shelter
    if (isLoggedIn && isShelter) {
      fetchPetListingData();
    }
  }, [listingId, isLoggedIn, isShelter, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Call the updatePetListing function from api.js to update the pet listing
      await updatePetListing(listingId, formData);

      // Handle the response, e.g., show a success message or redirect the user
      console.log('Pet listing updated successfully');

      // Redirect the user to the pet details page or other appropriate page
  ...
  return (
    <>
      ...
      <Container>
        <h1 className="text-center mt-4">Update Pet Listing</h1>
